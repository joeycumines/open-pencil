...
Quando l'app desktop è in esecuzione, i comandi CLI si connettono tramite il server MCP invece di richiedere un file .fig. Il server MCP è in ascolto su un socket di dominio Unix (o named pipe su Windows) con TCP opzionale su `127.0.0.1:7600` (HTTP + WebSocket). I comandi vengono eseguiti sullo stato dell'editor live, consentendo a script di automazione e agenti AI di interagire con l'app in esecuzione.
