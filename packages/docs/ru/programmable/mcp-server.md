...
Примечание: `eval` доступен через stdio, но отключён в режиме HTTP из соображений безопасности.
