...
Когда запущено десктоп-приложение, команды CLI подключаются к нему через сервер MCP вместо загрузки файла .fig. Сервер MCP слушает на Unix-сокете (или именованном канале в Windows) с опциональным TCP на `127.0.0.1:7600` (HTTP + WebSocket). Команды выполняются относительно состояния живого редактора, позволяя скриптам автоматизации и ИИ-агентам взаимодействовать с запущенным приложением.
