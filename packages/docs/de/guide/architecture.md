...
Wenn die Desktop-App läuft, verbinden sich CLI-Befehle über den MCP-Server statt eine .fig-Datei zu benötigen. Der MCP-Server lauscht auf einem Unix-Domain-Socket (oder Named Pipe unter Windows) mit optionalem TCP auf `127.0.0.1:7600` (HTTP + WebSocket). Befehle werden gegen den Live-Editor-Zustand ausgeführt, sodass Automatisierungsskripte und KI-Agenten mit der laufenden App interagieren können.
