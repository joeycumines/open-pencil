...
/**
 * Callback registered by core's instance-override system to flush its caches
 * when instance children are repopulated or components are swapped. The
 * scene-graph package is framework-agnostic and does not own the override
 * cache — core registers this hook at startup.
 */
let onInstanceStructureChange: (() => void) | null = null

export function setInstanceStructureChangeHandler(fn: (() => void) | null): void {
  onInstanceStructureChange = fn
}
