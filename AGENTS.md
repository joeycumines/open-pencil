...
- Transport architecture (`packages/mcp/src/transport/`): `paths.ts` resolves platform-specific socket dir/path (macOS `~/Library/`, Linux `XDG_RUNTIME_DIR`, Windows named pipe). `discovery.ts` writes/reads `mcp.json` discovery file with PID liveness check, stale socket cleanup, and auth token. Socket permissions `0o600` on Unix.
