...
The MCP server uses a Unix domain socket as the primary transport on macOS/Linux (named pipe on Windows), with optional TCP fallback. The stdio bridge (`openpencil-mcp`) connects via the socket; `openpencil-mcp-http` starts the TCP listener. Set `OPENPENCIL_MCP_TCP=1` to force TCP on Unix.
