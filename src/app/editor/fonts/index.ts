...
let tauriFontCacheHasHostLoader = false
...
function configureTauriFontCache() {
  // Stay configured once a Tauri context has attached the host loader, so a
  // later Tauri-mocked test cannot re-enter and leave a Tauri-specific
  // fetcher attached to the shared fontManager after the mock is torn down.
  // A plain (non-Tauri) latch is still upgradable: another module may import
  // this file before the Tauri-mocked tests run, and those tests must be able
  // to attach the host loader afterwards.
  if (tauriFontCacheConfigured && tauriFontCacheHasHostLoader) return
  tauriFontCacheConfigured = true
  if (!isTauri()) return
  tauriFontCacheHasHostLoader = true
