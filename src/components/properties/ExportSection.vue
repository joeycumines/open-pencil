...
async function doExport(settings: Array<{ scale: number; format: ExportFormat }>) {
  exporting.value = true
  try {
    for (const s of settings) await store.exportSelection(s.scale, s.format)
  } finally {
  ...
  >
    <div data-test-id="export-section" :class="sectionWrapper()">
      ...
      <div
        ...
        class="mt-1 overflow-hidden rounded border border-border"
      >
        <img
          :src="previewUrl"
          class="block w-full"
