...
The stdio bridge (`openpencil-mcp`) connects to the MCP server via the Unix socket (or named pipe) and sends the auth token on every request. It inherits the same auth requirements as direct connections.
