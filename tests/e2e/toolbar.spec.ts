...
test('Polygon tool creates POLYGON node', async () => {
  ...
  expect(children.some((n: any) => n.type === 'POLYGON')).toBe(true)
  canvas.assertNoErrors()
})
...
test('Star tool creates STAR node', async () => {
  ...
  const children = await getPageChildren(page)
  expect(children.some((n: any) => n.type === 'STAR')).toBe(true)
  canvas.assertNoErrors()
})
...
test('Pen creates VECTOR node with 3 vertices on Enter', async () => {
  ...
  const children = await getPageChildren(page)
  const vectors = children.filter((n: any) => n.type === 'VECTOR')
...
test('Pen Escape with 2 vertices cancels path without creating node', async () => {
  const before = (await getPageChildren(page)).filter((n: any) => n.type === 'VECTOR').length
  ...
  const after = (await getPageChildren(page)).filter((n: any) => n.type === 'VECTOR').length
...
test('Pen close path creates VECTOR with closed region', async () => {
  ...
  const vectors = (await getPageChildren(page)).filter((n: any) => n.type === 'VECTOR')
  const last = vectors[vectors.length - 1]
